...
namespace Microsoft.WindowsPhone.Imaging
{
	...
	public class FullFlashUpdateImage
	{
		...
		public FullFlashUpdateImage(string A_1) : this()
		{
			...
			using (FileStream imageStream = GetImageStream())
			{
				using (BinaryReader binaryReader = new BinaryReader(imageStream))
				{
					...
					{
						binaryReader.ReadBytes((int)CalculateAlignment((uint)imageStream.Position, _imageHeader.ChunkSize * 1024U));
					}
		...
		public static int ImageHeaderSize
		{
			get
			{
				return Marshal.SizeOf(new ImageHeader());
			}
		}

		public static int ImageHeaderExSize
		{
			get
			{
				return Marshal.SizeOf(new ImageHeaderEx());
			}
		}
		...
		public List<FullFlashUpdateStoragePool> StoragePools
		{
			get
			{
				return new List<FullFlashUpdateStoragePool>(_ffuStoragePools);
			}
		}
		...
		public FullFlashUpdateManifest Manifest { get; private set; }
		...
		private ImageHeaderEx _imageHeader = new ImageHeaderEx();
